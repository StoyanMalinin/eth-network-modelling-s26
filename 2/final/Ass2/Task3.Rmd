...
Third, the model could include additional **opportunity-structure covariates**. Students do not have equal opportunities to meet and become friends with all other students. Shared contexts such as class membership, same corridor, extracurricular activities, or shared leisure spaces could strongly shape friendship formation. Adding such dyadic covariates would help distinguish social preference from opportunity: students may appear to cluster not only because of endogenous transitivity, but also because they repeatedly encounter the same peers in structured settings. These covariates could improve the fit to geodesic distances and clustering patterns.
...
This hypothesis could be operationalized by defining a mutual friendship network that contains only reciprocated ties and then using this network for an average alter effect on alcohol. An `avAlt`-type effect based on the mutual friendship network would test whether students adjust their alcohol consumption toward the average alcohol consumption of reciprocated friends. A positive parameter would support the hypothesis that mutually acknowledged friendships are especially important channels of alcohol-related peer influence.
